...
| Pacote | Uso |
|---|---|
| `pandas>=2.2,<3` | Leitura tabular e suporte à leitura dos arquivos Excel, mantendo compatibilidade com `databricks-connect 18.x`. |
...
A execução deve terminar exibindo a versão de cada pacote. A restrição `pandas>=2.2,<3` é intencional: o log do ambiente mostrou que `databricks-connect 18.0.9` exige `pandas<3`. Se ocorrer erro de permissão durante `pip install`, use a instalação como biblioteca PyPI do compute ou Job, que é a opção recomendada para o ambiente Databricks.
