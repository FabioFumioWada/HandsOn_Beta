"""Ingestão Bronze específica do IBGE.
...
def list_source_files(path: str) -> List[Dict[str, Any]]:
    """Lista todos os arquivos IBGE, incluindo extensões não elegíveis."""
...
# Item 1: listagem de todos os arquivos iniciados por IBGE.
...
# Item 2: importar somente CSV/XLSX iniciados por IBGE.
...
# Registra arquivos IBGE fora do escopo no controle global.
...
# Item 2: cada conjunto IBGE vira uma tabela no schema bronze_ibge.
            ...
            dictionary_rows.append(
                {
                    ...
                    "finalidade_descricao": f"Campo carregado do conjunto IBGE {dataset_key}.",
...
print(f"Arquivos IBGE listados: {len(source_files)}")
print(f"Arquivos IBGE elegíveis importados: {len(eligible_files)}")
